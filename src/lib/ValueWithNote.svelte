<script lang="ts">
    ...
    export let solutionName: string;
...
<div class="flex flex-wrap">
    {#if item.value.support}
        <SupportLabel support={item.value.support}>
            <div
                class="opacity-0 group-hover:opacity-100 transition-opacity flex items-center ml-2 pl-1 border-l border-gray-300"
            >
                ...
                <div class="flex-1 text-gray-400">
                    <EditButton {solutionName} {feature} />
                </div>
            </div>
        </SupportLabel>
    {:else if isArray}
